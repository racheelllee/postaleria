 ...
 *
 */
class ElementTask extends BakeTask
{
    /**
    ...
    {	
    	$path_element = parent::getPath().'Element/';
        ...
        if(strpos($action,'_pdf') !== false){
            $path = $this->getPath().'pdf/';
            if(!file_exists($path)){
                @mkdir($path);
            }

            list($a,$x)=explode('_',$action);
            $action = Inflector::underscore($a);
        }else{
        	return "" ;
        }
